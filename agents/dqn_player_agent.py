...
class PlayerDQN(Algorithm):
  def __init__(self, model, target_model, loss_func, optimizer, buffer, device,
  ...
  def update(self, step_total):
    ...
    obs, action, reward, next_obs, done, weights \
        = obs.to(self.device), action.to(self.device), reward.to(self.device), next_obs.to(self.device), done.to(self.device), weights.to(self.device)
